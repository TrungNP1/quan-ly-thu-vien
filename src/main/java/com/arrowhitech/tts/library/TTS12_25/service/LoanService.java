...
public class LoanService {
    ...
    public LoanResponseDTO loan(LoanRequestDTO dto){
        ...
        if(loanRepository.existsByUserAndStatus(user, LoanStatus.OVERDUE)){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Bạn không thể mượn thêm vì đã để sách quá hạn không trả");
        }

        if (!book.getIsActive() || book.getAvailableCopies() <= 0) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Sách không khả dụng");
        }

        if(loanRepository.countByUserAndStatus(user, LoanStatus.BORROWING) >= 5){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Bạn không thể mượn thêm vì đã mượn 5 quyển sách");
        }

        if(loanRepository.findByUserAndBookAndStatus(user, book, LoanStatus.BORROWING).isPresent()){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Bạn không thể mượn thêm vì đang mượn quyển sách tương tự");
        }
    ...
    public LoanResponseDTO returned(Long id){
        ...
        .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Không tìm thấy thông tin mượn sách"));

        if(loan.getStatus() == LoanStatus.RETURNED){
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Sách đã được trả");
        }
